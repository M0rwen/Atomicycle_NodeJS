...
const saveTokenForUser = (userId, token) => {
    connection.execute('UPDATE users SET token = ? WHERE id = ?', [token, userId], (err) => {
        if (err) {
...
const getAllUsers = (req, res) => {
    const sql = 'SELECT id, name, email, role, token, deleted_at FROM users WHERE deleted_at IS NULL ORDER BY id DESC';

    connection.query(sql, (err, rows) => {
        if (err) {
            console.log(err);
            return res.status(500).json({ error: 'Error loading users', details: err });
        }
...
const deactivateUserById = (req, res) => {
    ...
    const sql = 'UPDATE users SET deleted_at = ?, token = NULL WHERE id = ?';
    connection.execute(sql, [timestamp, id], (err, result) => {
        if (err) {
            console.log(err);
            return res.status(500).json({ error: 'Error deactivating user', details: err });
        }
...
module.exports = { registerUser, loginUser, updateUser, deactivateUser, getAllUsers, updateUserRole, deactivateUserById }